  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
      ...
      zoom: function(scaleToAdd, globalRequestedPosition, duration) {
        // console.log( "scaleToAdd", scaleToAdd );
        // console.log( "globalRequestedPosition", globalRequestedPosition );
    ...
    function touchMoveHandler(e) {
      ...
      var smartData = targetElement.data('smartZoomData')
      // //console.log( smartData.moveLastPosition.toString() );
    ...
    function animate(target, globalLeft, globalTop, width, height, duration, callback) {
      ...
      if (smartData.transitionObject != null) { // use css transition if supported
        ...
        cssObject[smartData.transitionObject.transition] = 'all ' + duration / 1000 + 's' // set effect duration todo: ease-out

        if (smartData.transitionObject.css3dSupported) // use css 3d translate if supported
        { cssObject[smartData.transitionObject.transform] = 'translate3d(' + left + 'px, ' + top + 'px, 0) scale3d(' + width / originalSize.width + ',' + height / originalSize.height + ', 1)' } else { cssObject[smartData.transitionObject.transform] = 'translateX(' + left + 'px) translateY(' + top + 'px) scale(' + width / originalSize.width + ',' + height / originalSize.height + ')' }
    ...
    function getContainerDivSizeFromSettings(settingsValue, zoomableContainerParentValue, divPosDiff) {
      if (settingsValue.search && settingsValue.search('%') != -1) { return (zoomableContainerParentValue - divPosDiff) * (parseInt(settingsValue) / 100) } else { return parseInt(settingsValue) }
    }
    ...
    function getContainerDivPositionFromSettings(settingsValue, zoomableContainerParentPosValue, zoomableContainerParentSizeValue) {
      if (settingsValue.search && settingsValue.search('%') != -1) { return zoomableContainerParentPosValue + zoomableContainerParentSizeValue * (parseInt(settingsValue) / 100) } else { return zoomableContainerParentPosValue + parseInt(settingsValue) }
    }
